...
class EnhancedNLToMongoDBQuerySystem:
    ...
    def _generate_multi_collection_query(self, query_text: str, primary_collection: str, 
        ...
        # Get company filtering info
        company_field = self.company_filtered_collections.get(primary_collection, "companyId")
    ...
    - For primary collection ({primary_collection}), use field: {company_field}
    - Add company filter as first $match stage: {{"$match": {{"{company_field}": "COMPANY_ID_PLACEHOLDER"}}}}
    """
    ...
    """
        ...
        prompt = f"""
    You are a MongoDB aggregation expert. Generate a complex aggregation pipeline for this multi-collection query.
    ...
    1. NO COMMENTS of any kind in the JSON (no // or /* */)
    ...
    INSTRUCTIONS:
    1. Create an aggregation pipeline with $lookup stages to join collections
    2. Start with $match stage for filtering (including company filter)
    ...
    7. Add $group, $sort, or other stages as needed for the query requirements
    8. NEVER include comments in the JSON output

    RESPONSE FORMAT - Return ONLY valid JSON without any comments:
    {{
        "collection": "{primary_collection}",
        "query": [
            {{"$match": {{"{company_field}": "COMPANY_ID_PLACEHOLDER"}}}},
            {{"$lookup": {{"from": "target_collection", "localField": "local_field", "foreignField": "foreign_field", "as": "alias"}}}},
            {{"$unwind": {{"path": "$alias", "preserveNullAndEmptyArrays": true}}}},
            {{"$project": {{"field1": 1, "alias.field2": 1}}}},
            {{"$sort": {{"field": 1}}}}
        ]
    }}
    ...
    """
        ...
        try:
            ...
            print(f"Multi-Collection LLM Response: {response_text}")
    ...
    """
        ...
        # Fixed the prompt with proper escaping
        critical_aggregation_rule = """
    CRITICAL AGGREGATION RULES:
    9. For queries that need projection, transformation, or grouping, use AGGREGATION PIPELINE format: [{{"$match": {{...}}}}, {{"$project": {{...}}}}]
    10. For simple filtering queries, use FIND format: {{"field": "value"}}
    11. NEVER mix aggregation operators ($project, $group) with find query syntax
    12. If using $project, it MUST be inside an aggregation pipeline array
    """
    ...
    {{"collection": "user", "query": [{{"$match": {{{company_field}: "COMPANY_ID_PLACEHOLDER"}}}}, {{"$project": {{"name": 1, "email": 1}}}}]}}
...
""" 
    ...
    def process_query(self, natural_language_query: str, user_id: str, 
        ...
        # Phase 1: Select collections (this already works correctly)
        ...
        # SIMPLE LOGIC: If multiple collections selected → multi-collection query
        if len(selected_collections) > 1:
        ...
        # Fallback logic for case sensitivity (for single collection queries)
...
def main():
    ...
    result2 = system.process_query("how many customers are there in call interactions?", user_id)
