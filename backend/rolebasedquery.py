...
class NLToMongoDBQuerySystem:
    def __init__(self):
        ...
        self.company_field_mapping = {
            ...
            # Add more collections and their company field mappings
        }
...
def main():
    ...
    # OR if you have string representation:
    # company_id = "507f1f77bcf86cd799439011"  # Will be converted to ObjectId
 
    user_query = "How many users are there?"
 
    # Process query with company filtering
    result = nl_system.process_query(user_query, company_id=company_id)
 
    print("Query Result:")
    print(json.dumps(result, indent=2, default=lambda x: str(x) if isinstance(x, ObjectId) else x))
 
    # The generated query will automatically include company filtering with ObjectId
    # For example: {"$and": [{"callCount": {"$gte": 1}}, {"cId": ObjectId("507f1f77bcf86cd799439011")}]}
